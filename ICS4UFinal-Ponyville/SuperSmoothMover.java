...
public abstract class SuperSmoothMover extends Actor{
    ...
    public boolean isInWorld(){
        int cw = this.getImage().getWidth();
        int ch = this.getImage().getHeight();
        int ww = this.getWorld().getWidth();
        int wh = this.getWorld().getHeight();
        if(exactX-cw/2>ww || exactX+cw/2<0 || exactY-ch/2>wh || ch/2+exactY<0)
            return false;
        return true;
    }
